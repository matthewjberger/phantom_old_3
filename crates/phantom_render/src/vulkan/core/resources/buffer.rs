use crate::vulkan::core::{BufferToBufferCopyBuilder, CommandPool, Device};
...
impl Buffer {
    ...
    fn size(&self) -> u64 {
        if let Some(allocation) = self.allocation.as_ref() {
            ...
            log::warn!("Buffer size was requested while no allocation info was present.");
            0
        }
    }
}
    ...
    fn drop(&mut self) {
        if let Some(allocation) = self.allocation.take() {
            ...
            if let Err(error) = result {
                log::warn!("Failed to free allocated buffer: {}", error);
            }
        }
