...
impl Context {
    pub fn new<W: HasRawWindowHandle + HasRawDisplayHandle>(window_handle: &W) -> Result<Self> {
        let instance_extensions = Self::instance_extensions(window_handle.raw_display_handle())?;
    ...
    fn instance_extensions(raw_display_handle: RawDisplayHandle) -> Result<Vec<*const i8>> {
        let mut extensions: Vec<*const i8> =
            enumerate_required_extensions(raw_display_handle)?.to_vec();
