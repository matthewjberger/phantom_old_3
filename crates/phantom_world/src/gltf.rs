...
pub fn load_gltf(path: impl AsRef<Path>, world: &mut World) -> Result<()> {
    let (gltf, buffers, images) = gltf::import(path).map_err(GltfError::ImportGltfAsset)?;
...
fn load_nodes(
    ...
    for (index, node) in gltf.nodes().enumerate() {
        ...
        let mut entry = ecs.entry(entity).ok_or(GltfError::FindEntity)?;
...
fn load_primitive_vertices(
    ...
    let read_positions = reader
        .read_positions()
        .ok_or(GltfError::ReadVertexPositions)?;
...
fn load_animations(
    ...
    for animation in gltf.animations() {
        ...
        for channel in animation.channels() {
            ...
            let inputs = reader
                .read_inputs()
                .ok_or(GltfError::ReadChannelInputs)?
                .collect::<Vec<_>>();

            let outputs = reader.read_outputs().ok_or(GltfError::ReadChannelOutputs)?;
